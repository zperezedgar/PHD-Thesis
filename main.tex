...
\subsubsection{A Revision to the Time Evolution of the Free Wave Packet}
...
Thereby, we rewrite here the equations that we are going to use to establish the correspondence between terms

2- Terminar... (dividir ecs en dos columnas)
...
\begin{equation}
\begin{aligned}
& \sigma_{z}(t) \longrightarrow \sigma_{x}(\tau) \\ 
& 4 \beta \longrightarrow \epsilon \\ 
& \upsilon_{g} \longrightarrow 2 \rho_{0} \epsilon \\ 
\end{aligned}
\end{equation}


On the other hand, by comparing the second row of Eq. \ref{free_wave_packet_position_space_centered_z0_final_form} and the second row of Eq. \ref{alkali_wave_packet_position_space_final_form}, we can see that there exist the following correspondence

\begin{equation}
\omega_{0} \longrightarrow q_{1},
\end{equation}

Meanwhile for the rest of terms, we can see that some of them are present in one wave packet bu they are not present in the other and vice-versa while other terms look similar in both wave packets but they differ in some way that is not possible to establish a direct connection between them. Incidentally, the terms of the second row in both Eqs. \ref{free_wave_packet_position_space_centered_z0_final_form} and \ref{alkali_wave_packet_position_space_final_form} are the terms that represent the phase while the terms in the first rows are those terms that does not contribute to the phase.
