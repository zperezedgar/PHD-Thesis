...
\varphi_{\pm, \tau_{1}}(x) =-\tau \bigg(q_{1} + \frac{\epsilon}{3} q_{2_{\pm}}^{2} \tau^{2}\bigg) + (\rho_{0} - q_{2_{\pm}} \tau)(x-\frac{v_{0} \tau}{2}) \\
...
\varphi_{\pm, \tau_{2}}(x) = -(2\tau) \bigg(q_{1} + \frac{\epsilon}{3} q_{2_{\mp}}^{2} (2\tau)^{2}\bigg) + ((\rho_{0} - q_{2_{\pm}} \tau)-q_{2_{\mp}} (2\tau))(x-\frac{(v_{0}+a_{\pm}\tau) (2\tau)}{2})\\ 
...
\varphi_{\pm, \tau_{3}}(x) = -\tau \bigg(q_{1} + \frac{\epsilon}{3} q_{2_{\pm}}^{2} \tau^{2}\bigg) + ((\rho_{0} - q_{2_{\pm}} \tau-2q_{2_{\mp}}\tau)-q_{2_{\pm}} \tau)(x-\frac{(v_{0}+a_{\pm}\tau + 2a_{\mp}\tau) \tau}{2}) \\
...
Before proceeding to the final computation, we will write the inverse of $\sigma_{x}(\tau)$ as follows

\begin{equation}\label{sigma_x_beta}
\frac{1}{\sigma_{x}(\tau)} = \frac{\left[1 + \beta^{2}\right]^{-1/2}}{\sigma_{x}(0)},
\end{equation}
...
By using a Maclaurin expansion for Eq.\ref{sigma_x_beta} in terms of $\beta$, we can see that the approximation used in the last section (Eq.\ref{sigma_zero_approx}) was just the zeroth-order expansion of $1/\sigma_{x}(\tau)$. In order to get a correction, we will use the first-order expansion of Eq.\ref{sigma_x_beta}, i.e.,

\begin{equation}
\frac{1}{\sigma_{x}(\tau)} \approx \frac{\left[1 - \frac{1}{2}\beta^{2}\right]}{\sigma_{x}(0)},
\end{equation}

Therefore, at first order we can write

\begin{equation}\label{sigma_x_t1_first_order}
[\sigma_{x, \tau_{1}}]^{-1} = [\sigma_{x, \tau_{0}}]^{-1} \left[1 - \frac{1}{2}\bigg(\frac{\epsilon \tau}{[\sigma_{x, \tau_{0}}]^{2}}\bigg)^{2}\right],
\end{equation}

\begin{equation}\label{sigma_x_t2_first_order}
[\sigma_{x, \tau_{2}}]^{-1} = [\sigma_{x, \tau_{1}}]^{-1} \left[1 - \frac{1}{2}\bigg(\frac{\epsilon \tau}{[\sigma_{x, \tau_{1}}]^{2}}\bigg)^{2}\right],
\end{equation}

\begin{equation}\label{sigma_x_t3_first_order}
[\sigma_{x, \tau_{3}}]^{-1} = [\sigma_{x, \tau_{2}}]^{-1} \left[1 - \frac{1}{2}\bigg(\frac{\epsilon \tau}{[\sigma_{x, \tau_{2}}]^{2}}\bigg)^{2}\right].
\end{equation}

Finally, by using Eqs.(\ref{quantum_phase_ignored_terms_simplified_t1}, \ref{quantum_phase_ignored_terms_simplified_t2}, \ref{quantum_phase_ignored_terms_simplified_t3}) alongside Eqs.(\ref{sigma_x_t1_first_order}, \ref{sigma_x_t2_first_order}, \ref{sigma_x_t3_first_order}) we get the first order correction to the gravimetry signal
