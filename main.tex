...
[\sigma_{x, \tau_{1}}]^{-2} \approx [\sigma_{x, \tau_{0}}]^{-2}\left[1 - \bigg(\frac{\epsilon (\tau+2\tau)}{[\sigma_{x}(0)]^{2}}\bigg)^{2}\right] = [\sigma_{x, \tau_{0}}]^{-2}\left[1 - 9\beta^{2}\right],
\end{equation}
...
[\sigma_{x, \tau_{1}}]^{-2} \approx [\sigma_{x, \tau_{0}}]^{-2}\left[1 - \bigg(\frac{\epsilon (\tau+2\tau+\tau)}{[\sigma_{x}(0)]^{2}}\bigg)^{2}\right] = [\sigma_{x, \tau_{0}}]^{-2}\left[1 - 16\beta^{2}\right].
\end{equation}
...
\begin{multline}\label{gravimetry_signal_first_order_approx}
...
The correction given by the first-order approximation goes like $\beta ^2$. Therefore, the error made by considering the first-order approximation will be set by $\beta ^2$ which for the case of typical experimental values (see description of Fig. \ref{phase_graph_first_order}), has a value of $\beta ^2 \approx 1.3 \times 10^{-5}$.
Something interesting about this result is that for times of the order of magnitude about the milliseconds, the phase value obtained by using the first-order approximation is approximately half the value we obtain by using the zeroth-order approximation (see description of Fig. \ref{phase_graph_first_order}). Nonetheless, the order of magnitude is the same.


\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{phase1ordervs2order.png}
\caption{Comparison of the gravimetry signal obtained by using the zeroth-order approximation given by Eq. \ref{quantum_gravimetry_signal} versus the first-order correction given by Eq. \ref{gravimetry_signal_first_order_approx_dimensional}. The period used to produce this graph was $T=100ms$. Besides, we used $g=9.8m/s^{2}$, $\mu=9.27\times10^{-24} J/T$ (the Bohr magneton), $\eta=0.05 T/m$. Additionally, we consider $\ce{^{87}_{}Rb}$ atoms, so we have (\cite{Bunge1993}\cite{KAUSHALSK1970}) $M=1.45\times10^{-25}kg$, $\Delta W=2\pi\times6.835\times10^{9}s^{-1}$ ($\hbar \Delta W $ is the field-free ground state hyperfine energy splitting). Also, we use the approximations in Eqs. \ref{kappa_approx} and \ref{gamma_1_approx}. In addition, we take $p_{0}=0$, $z_{0}=0$, $\Delta Z(0)=100\times10^{-6}m$, and take $z$ to be the final position of the center of the wave packet at the end of the experiment. The center of the wave packet was evaluated by using the Eq. \ref{xi_eq_motion}, where $x=\kappa z$. In this case, $(\sigma_{x, \tau_{0}})^{2} \approx 4.1 \times 10^{-10}$ and $\epsilon \tau \approx 1.5 \times 10^{-11} t$. Thus, for $t=100ms$, we have $\beta \approx 3.6 \times 10^{-3}$ and the expansion used in the Eqs. \ref{sigma_x_t1_first_order}-\ref{sigma_x_t3_first_order} is justified. Moreover, the gap between both approximations will be proportional to $\beta ^2 \approx 1.3 \times 10^{-5}$. Finally, it can be noticed that in this regime of time the phase obtained by using the first-order correction is approximately half the phase we obtained using the zeroth-order approximation.}
