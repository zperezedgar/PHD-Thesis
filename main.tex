...
If we consider the wave function of the atom to be symmetric around $z_{0}$, for example by considering a gaussian like wave packet, we can define the half width at half maximum and name this point as $z_{HWHM}$ (see Fig. )(PONER GRAFICA AQUI DEFINIENDO Z-HMHM). Then, the width of our atom will be given by

\begin{equation}
  \Delta Z =  z_{0} + z_{HWHM}.
\end{equation}

Now, if we evaluate the detuning at $z = z_{0} + z_{HWHM}$, we get

\begin{equation}
  \delta (\Delta Z) = \frac{\mu_{B} g_{j} m_{j} \eta}{\hbar} z_{HWHM}
\end{equation}

i.e.,

\begin{equation}
  z_{HWHM} = \delta (\Delta Z) \frac{\hbar}{\mu_{B} g_{j} m_{j} \eta}.
\end{equation}

Thereby, to determine the width of our atom's wave function, we just need to determine the detuning at $z = z_{0} + z_{HWHM}$. In order to do so, firstly, we recall that the probability of finding the atom in the excited state at time $t$ when it was initially in the ground state at $t=0$ is given by
...
(PONER GRAFICA AQUI $|c_{e}(t)|^{2}$ VS $\delta$)

Now, a physical argument comes into play. We need to evaluate the detuning at $\Delta Z$. We will consider the detuning evaluated at $\Delta Z$ to be such that the probability of transition at this point will be zero. Furthermore, we will take the probability of transition starting from this point to be zero. In other words, we are making the assumption 

\begin{equation}
\sqrt{\big[ \delta (\Delta Z)\big]^{2} + \Omega^{2}} \bigg( \frac{t}{2} \bigg) = \pi,
\end{equation}

and also $|c_{e}(t)|^{2}=0$ for $|\delta (\Delta Z)| > |\delta (\Delta Z)|$.

(PONER GRAFICA AQUI $|c_{e}(t)|^{2}$ VS $\delta$ cortada en el primer cero)

This approximation makes sense because the detuning evaluated at positions farther away from the position that defines the width of the atom ($z_{HWHM}$) should be such that the probability of transition from that position tends to zero abruptly. In other words, we cannot induce transitions in the atom if the perturbation is applied at positions where the probability of finding the atom is zero.
