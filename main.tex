...
\begin{equation}
\varphi(\tau) = \Theta_{p}(x, \tau) - \frac{1}{2}\arctan\left[\frac{\epsilon \tau}{(\sigma_{x}(0))^{2}}\right]
\end{equation}

or more explicitly, using Eq.\ref{theta_p}

\begin{multline}\label{quantum_phase}
\varphi(\tau) = -\tau \bigg(q_{1} + \frac{\epsilon}{3} q_{2}^{2} \tau^{2}\bigg) + \left[\frac{\sigma_{x}(0)}{\sigma_{x}(\tau)} \right]^{2} (\rho_{0} - q_{2} \tau)(x-\rho_{0} \epsilon \tau) \\
+ \frac{\epsilon \tau}{4 [\sigma_{x}(\tau)\sigma_{x}(0)]^{2}} \left[(x-x_{0})^{2} + 4\rho_{0} x_{0} \epsilon \tau -2q_{2} \epsilon \tau^{2} \bigg(x+x_{0}- \frac{q_{2}}{2} \epsilon \tau^{2} \bigg)\right] 
...
\varphi_{\pm}(\tau) \approx -\tau \bigg(q_{1} + \frac{\epsilon}{3} q_{2}^{2} \tau^{2}\bigg)
\end{equation}
...
Therefore, the phase difference accumulated during the first interval is given by 

\begin{equation}
\Delta \Phi_{T_{1}} = \varphi_{+}(\tau) - \varphi_{-}(\tau)
\end{equation}
...
\begin{equation}
\Delta \Phi_{T_{2}} = [\varphi_{+}(4\tau) - \varphi_{+}(3\tau)] - [\varphi_{-}(4\tau) - \varphi_{-}(3\tau)]
\end{equation}
...
Now, we will make additional some approximations. First,


by using Eqs.(\ref{approx_quantum_phase}-\ref{pre_total_quantum_phase}) and the definitions Eqs.(\ref{epsilon}-\ref{gamma_1}) we obtain

\begin{equation}\label{quantum_gravimetry_signal}
\Delta \Phi = 8 \frac{\mu \eta g \tau^{3}}{\hbar (\Delta W)^{3}}
\end{equation}

and by remembering that the non-dimensional time is defined as $\tau\equiv \Delta W t$, we see that this result agrees with the result obtained by using the semi-classical picture Eq.\ref{gravimetry_signal}.
