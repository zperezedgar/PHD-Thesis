...
In the same way, the width at the end of the third interval (Eq. \ref{sigma_x_t3}) will be given in first-order by 
...
The correction given by the first-order approximation goes like $\beta ^2$. Therefore, the error made by considering the first-order approximation will be set by $\beta ^2$ which for the case of typical experimental values (see description of Fig. \ref{phase_graph_first_order}), has a value of $\beta ^2 \approx 3.3 \times 10^{-10}$. As we can see in Fig. \ref{fig:gravimetry_signal_zeroth_order}, this gravimetry signal could allow us to measure $g$ with $9$ digits of precision, however, according to the Fig. \ref{fig:gravimetry_signal_first_minus_zeroth_order} the error due to the first-order correction would start affecting the eighth digit of the measurement. Therefore, in reality, we should be able to measure $g$ with $8$ digits of precision. This precision is affected principally by the initial width of the wave packet as can be seen in the definition \ref{beta_parameter}. For every order of magnitude that we increase(decrease) the initial width of the wave packet, we gain(lose) $4$ digits of precision in the measurement because the correction goes like $\beta ^2$.
...
Now, we will re-write the wave packet of alkali atoms (Eq. \ref{wave_packet_non_dimensional_evolution}) but this time, in order to establish the connection with the free wave packet, we will ignore the terms that include $q_{2}$ since this parameter is related with the acceleration of the wave packet (see Eq. \ref{xi_defs_eq_motion}) and for that reason, those terms do not have an analog in the free wave packet. Therefore, the wave packet of alkali atoms (ignoring terms that include $q_{2}$) will be given by
...
Observe that $2 \rho_{0} \epsilon$ is related to the velocity of the center of the wave packet as we already shown in Eq. \ref{xi_defs_eq_motion}.
...
This is not surprising since we already knew that $\rho_{0}=p_{0}/\hbar \kappa$. Although the correspondence is not directly due to the presence of the expansion term in the alkali wave packet.

Meanwhile, for the rest of the terms, we can see that some of them are present in one wave packet but they are not present in the other and vice-versa while other terms look similar in both wave packets but they differ in some way that is not possible to establish a direct connection between them. Incidentally, the terms of the second row in both Eqs. \ref{free_wave_packet_position_space_centered_z0_final_form} and \ref{alkali_wave_packet_position_space_final_form} are the terms that contribute to the phase while the terms in the first rows are those terms that do not contribute to the phase.
...
In order to do so, first, we will establish the theoretical framework to describe the initialization of the atom to be used during the interferometry procedure. Specifically, we will show that inducing Rabi transitions in the hyperfine levels of an atom placed inside an inhomogeneous magnetic field causes its position wave function to have a specific width when the initial width is larger than the selection range set by the power of the microwaves. Then, we will calculate the initial width of the atoms just before the measurement process by considering a cloud of alkali atoms in thermal equilibrium. Finally, in the next sections, we will use these results to choose the best possible initial width.
...
We will consider transitions between two hyperfine state levels of the atom with a different magnetic quantum number.\footnote{For example, we can choose the ground state to be the state with total angular momentum number $F=0$ and the magnetic number $m_{F}=0$ while the excited state could be that with $F=1$ and $m_{F}=1$, i.e.,
...
where $\sigma_{z}(0)$ is the initial standard deviation of the distribution defined in Eq. \ref{free_wave_packet_width}.
...
Let's assume that the atom's wave function is centered at $z_{0}$\footnote{Later, we will establish the condition that allows our method to be independent of the initial position of the atom, so we do not have to worry about the case where the atom has an initial offset from the origin.}, the point where the detuning is zero, and take it as the origin of coordinates, i.e., $z_{0}=0$. Therefore, if we evaluate the detuning at $z = 2Z_{HWHM}$, we get
...
We remember that the electric field is several orders of magnitude larger than the magnetic field, thereby it dominates, and the Rabi flopping frequency is usually taken to be

\begin{equation*}
\Omega = \frac{- e E_{0} \bra{e} r \ket{g}}{\hbar},
\end{equation*}

where $r$ is the position operator in the direction of the polarization of the electric field, $E_{0}$ is the amplitude of the electric field, and $e$ is the electron charge. Nonetheless, we will be considering hyperfine levels (Eq. \ref{hyperfine_levels}) for which the matrix elements of the position operator will vanish  \cite{Steck2010}. Therefore, the Rabi flopping frequency will depend solely on the magnetic field, i.e.,
...
This approximation and the exact form of the transition probability versus the detuning can be seen in Fig. \ref{ce2_VS_detuning}.
...
Now, we will solve Eqs. \ref{bloch_eq_pos_space} and \ref{bloch_eq_initial_cond_pos_space} for the case of a harmonic oscillator Hamiltonian, i.e.,
...
We remember that the time-evolution of the matrix elements of the density operator is given by
...
Let's use the above equations to get more insight into the experimental values that we can use to get the best results. Experimentally, besides the time $t$, we can only adjust the angular frequency $\omega$ and the temperature of the trap $T$. However, for an experimental point of view, it makes more sense to fix the temperature and adjust the frequency as needed. Let's analyze what happens with the expansion in different temperature regimes.

\vspace{5mm}
\textbf{Low-Temperature Regime}

If we try to optimize Eq. \ref{atomic_dispersion_with_gravity_3} to get the initial width that minimizes it. The optimal initial width is trivially $(\Delta Z(0))^{*}=0$. However, according to Eq. \ref{atomic_dispersion_with_gravity_t0}, obtaining an initial width of zero is impossible. Nonetheless, we can still calculate the minimum possible initial width if we optimize Eq. \ref{atomic_dispersion_with_gravity_t0}. The initial width is minimized when the hyperbolic cotangent reaches its minimum value. However, the hyperbolic cotangent does not have a global or local minimum. Although this function does not have a minimum, when the $\coth(x)$ function is defined over the interval $[0,\infty)$, it reaches its minimum value at infinity because $\lim_{x^{+} \to 0} \coth(x)=\infty$ and $\lim_{x \to \infty} \coth(x)=1$. Therefore, this function will have its minimum value at infinity even though it cannot be considered a global minimum in the usual sense because infinity is not a number. In our case, this point corresponds to $T= 0$. Thus, at $T=0$ the initial width reaches its minimum value and becomes

\begin{equation}
    \Delta Z(0) = \sqrt{\frac{\hbar}{2M \omega} } , \quad (T=0).
\end{equation}

Obviously, it is not possible to obtain that temperature experimentally. However, we can still use this result as an approximation for the low-temperature regime, as we will see later. Thus, we have that for low temperatures, the initial width is approximately given by

\begin{equation}\label{delZ0_smallT}
    \Delta Z (0) \approx \sqrt{\frac{\hbar}{2M \omega} } , \quad (\hbar \omega \gg kT).
\end{equation}
...
If we substitute the initial width for the low-temperature regime in Eq. \ref{atomic_dispersion_with_gravity_3}, we get that the expansion is given by

\begin{equation}\label{expansion_low_temp}
    \Delta Z (t) = \sqrt{\frac{\hbar}{2M \omega} }  \sqrt{1+t^{2}\omega^{2}}, \quad (\hbar \omega > 2kT) ,
\end{equation}

Notice that in this low-temperature regime, the expansion is independent of the temperature. This last equation can be optimized as a function of $\omega$. The frequency that minimizes the previous equation is given by

\begin{equation}\label{optimal_freq_low_temp}
    \omega^{*} = \frac{1}{t}.
\end{equation}

For example, for a total time of $t=100ms$, the optimal frequency is $\omega^{*}=10 Hz$. If we substitute Eq. \ref{optimal_freq_low_temp} into Eq. \ref{expansion_low_temp}, we obtain the expansion for the optimal frequency, i.e.,

\begin{equation}\label{minimal_expansion_low_temp}
    \Delta Z_{min}  (t)= \sqrt{\frac{\hbar t}{M} } , \quad (\hbar \omega > 2kT , \quad \omega = 1/t).
\end{equation}

For a typical experimental temperature of $T\approx 1 \mu K$ and an optimal frequency of $\omega^{*}=10 Hz$ ($t=100ms$), this result is an excellent approximation for the expansion in the low-temperature limit.

\vspace{5mm}
\textbf{High-temperature Regime}

In the high-temperature regime ($\hbar \omega \gg kT$), we can expand the hyperbolic cotangent in Eq. \ref{atomic_dispersion_with_gravity_t0} using a Maclaurin series. Thus, in this limit, the initial width can be written as

\begin{equation}
    \Delta Z (0) \approx \sqrt{\frac{ kT}{ M \omega^{2}}  }, \quad (\hbar \omega \ll kT).
\end{equation}

We can use this approximation to rewrite the expansion in Eq. \ref{atomic_dispersion_with_gravity_3} as

\begin{equation}\label{minimal_expansion_high_temp}
    \Delta Z (t) = \sqrt{\frac{ kT}{ M \omega^{2}} } \sqrt{1+t^{2}\omega^{2}}, \quad (\hbar \omega \ll kT).
\end{equation}

This equation does not have a minimum with respect to $\omega$. Indeed, the largest the frequency, the smaller the expansion. Therefore, in this regime, to get a minimal expansion, the frequency has to have the largest possible value. Note that for long expansion times, we can approximate the last result as

\begin{equation}\label{minimal_expansion_high_temp_long_time}
    \Delta Z (t) \approx \sqrt{\frac{ kT}{ M } }t, \quad (\hbar \omega \ll kT \quad and \quad 1 \ll \omega t ).
\end{equation}

\vspace{5mm}
\textbf{General Result}

The result in Eq. \ref{atomic_dispersion_with_gravity_2} is general and valid for all temperatures. Anyways, we can still try to optimize it. As we mentioned before, experimentally, it makes more sense to fix the temperature and adjust the frequency as needed. However, it is not possible to find analytically, the value for $\omega$ that minimizes the expansion in Eq. \ref{atomic_dispersion_with_gravity_2}, although, from a plot, it can be easily seen that there exists a minimum. Therefore, the value for $\omega$ that minimizes the expansion of the width will have to be found numerically, and it  will depend on the total time. For example, for a $\ce{^{87}_{}Rb}$ atom and a temperature of $T=1\mu K$, the frequency that minimizes the width at $t=100ms$ is given by
...
We can use the results of the last section, to get the total time needed to reach a given width.

\vspace{5mm}
\textbf{Low-Temperature Regime}

The more general result, valid for any frequency in the low-temperature regime, can be obtained by solving Eq. \ref{expansion_low_temp}, i.e.,

\begin{equation}\label{dipole_trap_total_time_low_temp}
    t = \frac{1}{\omega}\sqrt{\frac{2M \omega}{\hbar} \Delta Z ^{2} - 1}, \quad (\hbar \omega > 2kT).
\end{equation}

For typical experimental values $\omega \sim 10kHz$. Thus, we can approximate the last equation for high-frequency values as\footnote{For a $\ce{^{87}_{}Rb}$ atom, $\omega = 10kHz$, and $\Delta Z=100\mu m$, we have that $\frac{2M \omega}{\hbar} \Delta Z ^{2} \approx 27*10^{3}$.}

\begin{equation}\label{dipole_trap_total_time_low_temp_high_freq}
    t \approx \sqrt{\frac{2M}{\hbar \omega} }  \Delta Z, \quad (\hbar \omega > 2kT).
\end{equation}

Note that, if the frequency is chosen according to Eq. \ref{optimal_freq_low_temp}, the total time needed to reach a  width $\Delta Z$ can be obtained by solving Eq. \ref{minimal_expansion_low_temp}, i.e.,

\begin{equation}
    t_{max} = \frac{M}{\hbar} \Delta Z ^{2}, \quad (\hbar \omega > 2kT , \quad \omega = 1/t).
\end{equation}

This is the maximum expansion time that could be obtained in the low-temperature regime. For example, for a $\ce{^{87}_{}Rb}$ atom and a final width of $\Delta Z=10\mu m$, the total time would be around $t=137ms$. However, according to Eq.  \ref{optimal_freq_low_temp}, this means that the trap would have to have a frequency value of around $\omega \sim 10Hz$. This frequency value is two orders of magnitudes smaller than the usual frequency values used in dipole traps so, in the next sections, we will prefer to use Eq. \ref{dipole_trap_total_time_low_temp_high_freq} as the equation that describes the total expansion time in the low-temperature regime. Lately, we will see how to optimize the expansion time in the low-temperature and high-frequency regimes.

\vspace{5mm}
\textbf{High-temperature Regime}

In this limit, we have to solve Eq. \label{minimal_expansion_high_temp} for $t$. In this case, the total time to reach a width $\Delta Z$ is given by

\begin{equation}
    t = \frac{1}{\omega}\sqrt{\frac{M \omega^{2}}{kT} \Delta Z ^{2} - 1}, \quad (\hbar \omega \ll kT).
\end{equation}

As before, the largest the frequency, the largest the total time to reach the width $\Delta Z$. Therefore, we have to choose the largest possible frequency to get the largest possible expansion time. For long expansion times, we can approximate the total time by solving Eq. \ref{minimal_expansion_high_temp_long_time}, i.e.,

\begin{equation}
    t \approx \sqrt{\frac{M}{k T}} \Delta Z, \quad (\hbar \omega \ll kT, \quad 1 \ll \omega t ).
\end{equation}
...
The initial width can be written explicitly, i.e.,

\begin{equation}\label{t_dipole_trap_general}
...
\begin{equation}\label{max_time_free_fall}
    t_{max} = \sqrt{\frac{ 2 z_{max}}{g}}.
\end{equation}

For our purposes, we are interested in the low-temperature regime. Thus, in order to find out the constraint in the frequency that can be chosen for the trap, we use Eqs. \ref{dipole_trap_total_time_low_temp_high_freq} and \ref{max_time_free_fall} Eq. to get the following inequality

\begin{equation}\label{frequency_condition_dipole_trap}
    \begin{split}
        \sqrt{\frac{2M}{\hbar \omega}} \Delta Z &\leq \sqrt{\frac{ 2 z_{max}}{g}}, \\
    \omega&\geq g \frac{M}{\hbar} \frac{\Delta Z ^{2}}{z_{max}}.
    \end{split}
\end{equation}

For example, for a $\ce{^{87}_{}Rb}$ atom, a final width of $\Delta Z=100\mu m$, and a maximum free-fall distance of $z_{max}=30cm$, we get that the frequency has to be such that $\omega \geq 447 Hz$.

Note that Eq. \ref{frequency_condition_dipole_trap} does not only represent the constraint in the frequency value that can be chosen but also gives a way to choose the frequency such that the expansion time is maximized with respect to the maximum free-fall distance. Indeed, if we take the equality in Eq. \ref{frequency_condition_dipole_trap}, the optimized expansion time in the low-temperature and high-frequency regime will be given by Eq. \ref{max_time_free_fall}, i.e.,

\begin{equation}\label{dipole_trap_total_time_low_temp_high_freq_optimized}
    t_{max} = \sqrt{\frac{2M}{\hbar \omega^{*}}} \Delta Z
\end{equation}

where the optimal frequency as a function of the maximum free-fall distance is given by

\begin{equation}\label{optimal_frequency_low_temp_high_freq_dipole_trap}
    \omega^{*} = g \frac{M}{\hbar} \frac{\Delta Z ^{2}}{z_{max}}.
\end{equation}

\begin{comment}
For our purposes, we are interested in the low-temperature regime. Thus, in order to find out the constraint in the frequency that can be chosen for the trap, we use Eqs. \ref{dipole_trap_total_time_low_temp} and \ref{max_time_free_fall} Eq. to get the following inequality

\begin{equation}
    \begin{split}
        \frac{1}{\omega}\sqrt{\frac{2M \omega}{\hbar} \Delta Z ^{2} - 1} &< \sqrt{\frac{ 2 z_{max}}{g}}, \\
    \omega^{2} -\omega \frac{Mg}{\hbar z_{max}} \Delta Z ^{2} + \frac{g}{2 z_{max}}&>0.
    \end{split}
\end{equation}

The solution to this inequality is an interval given by the following inequalities
...
Or using interval notation and taking into account that $\omega>0$, the solution is

\begin{equation}
    \omega \in (0, \omega_{-}) \cup (\omega_{+}, \infty)
\end{equation}

where $\omega_{\pm}$ is given by

\begin{equation}
    \omega_{\pm} =  \frac{Mg}{\hbar z_{max}} \Delta Z ^{2} \pm \sqrt{\bigg( \frac{Mg}{\hbar z_{max}} \Delta Z ^{2} \bigg)^{2} - \frac{2g}{z_{max}}}
\end{equation}
\end{comment}
...
If instead of initializing the initial width using a selection beam, we use the expansion of the dipole trap. We can substitute Eq. \ref{warning_range} into Eq. \ref{dipole_trap_total_time_low_temp_high_freq} to get

\begin{equation}
    t_{max} = \sqrt{\frac{2M}{\hbar \omega}} \gamma \Delta Z',
\end{equation}

If we substitute Eq. \ref{maximum_selection_range} into this last equation, we get\footnote{We have taken the absolute value of Eq. \ref{maximum_selection_range}.}

\begin{equation}
    t_{max} = \sqrt{\frac{12M \mu_{0} P}{\hbar \omega c A}} \frac{ \gamma \bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F_{eff}} m_{F_{eff}} \eta}.
\end{equation}

Thus, using the approximation in Eq. \ref{t_max_vs_T} and Eq. \ref{gravimetry_signal_T}, we get the gravimetry signal obtained when using the dipole trap to select the initial width, i.e.,

\begin{equation}\label{gravimetry_signal_irradiance_dipole_trap}
    \Delta \Phi = b_{d} g I^{3/2}, 
\end{equation}

where $I$ is the irradiance of the microwaves, and we have defined the parameter

\begin{equation}
    b_{d} \equiv \frac{4}{\hbar} \frac{\mu_{B} \gamma^{3}}{\eta^{2}} \bigg( \frac{\bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F_{eff}} m_{F_{eff}}} \sqrt{\frac{3}{4} \frac{M}{\hbar \omega} \frac{\mu_{0}}{c} } \bigg)^{3}.
\end{equation}

The above result is general, however, we can opt to use the optimal frequency value ($\omega^{*}$) so the expansion time is as large as possible. Once we have decided on the experimental parameters (except for the frequency of the dipole trap), we can use Eq. \ref{optimal_frequency_low_temp_high_freq_dipole_trap} to compute the optimal frequency. In this case, $\Delta Z$ is given by Eq. \ref{warning_range}, thus, we will have that

\begin{equation}
    \omega^{*} = g \frac{M}{\hbar} \frac{(\gamma \Delta Z') ^{2}}{z_{max}},
\end{equation}

and by using \ref{maximum_selection_range}, we get that the optimal frequency is given by

\begin{equation}
    \omega^{*} = g \frac{M}{\hbar} \frac{\gamma^{2}}{z_{max}} \frac{6 \mu_{0} I}{c} \bigg(\frac{\bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F_{eff}} m_{F_{eff}} \eta} \bigg)^{2}.
\end{equation}

\begin{comment}
\textbf{Frequency Fixed}

If instead of initializing the initial width using a selection beam, we use the expansion of the dipole trap in the low-temperature and high-frequency regimes. We can substitute Eq. \ref{warning_range} into Eq. \ref{t_dipole_trap_optimized} to get
...
\begin{equation}
    b_{d_{T}} \equiv \frac{\mu_{B} \eta}{16 \hbar \omega^{3}} \bigg(6 \frac{M \omega^{2}}{kT} \frac{\mu_{0} I}{c} \bigg(\frac{ \gamma  \bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F_{eff}} m_{F_{eff}} \eta}\bigg)^{2} - 1\bigg)^{3/2},
\end{equation}
...
In the same way, if we choose to keep the temperature fixed instead of the frequency, we can use Eq. \ref{t_dipole_trap_general} as the equation that sets the available time to perform the experiment. If we substitute Eq. \ref{warning_range} into Eq. \ref{t_dipole_trap_general}, we get

\begin{equation}\label{expansion_time_dipole_trap_general}
    t = \frac{1}{\omega} \sqrt{\frac{12 M \omega}{\hbar} \tanh \bigg( \frac{\hbar \omega}{2 K T} \bigg) \frac{\mu_{0} P}{ c A} \bigg(\frac{ \gamma  \bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F_{eff}} m_{F_{eff}} \eta}\bigg)^{2} -1 }.
\end{equation}

This equation can be maximized by choosing the optimal frequency. However, as we saw earlier, this equation has to be maximized numerically. For example, choosing a microwave power of $1W$, a temperature of $1\mu K$, and using the same values chosen in Fig. \ref{precision_vs_power_figure}, we get that the dipole trap frequency that maximizes this equation is

\begin{equation*}
    w^{*} \approx 21.6 kHz.
\end{equation*}

This frequency gives an expansion time of

\begin{equation}\label{optimal_expansion_time_temperature_fixed}
    t^{*} \approx 0.97 ms.
\end{equation}

We can use Eq. \ref{expansion_time_dipole_trap_general} alongside the approximation in Eq. \ref{t_max_vs_T} and Eq. \ref{gravimetry_signal_T}, to get gravimetry signal, i.e.,

\begin{equation}\label{gravimetry_signal_irradiance_dipole_trap_general}
    \Delta \Phi = b_{d_{\omega}} g, 
\end{equation}

where we have defined the parameter

\begin{equation}
    b_{d_{\omega}} \equiv \frac{\mu_{B} \eta}{16 \hbar \omega^{3}} \bigg(\frac{12 M \omega}{\hbar} \tanh \bigg( \frac{\hbar \omega}{2 K T} \bigg) \frac{\mu_{0} P}{ c A} \bigg(\frac{ \gamma  \bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F_{eff}} m_{F_{eff}} \eta}\bigg)^{2} -1 \bigg)^{3/2},
\end{equation}

\subsubsection{Selection of the Temperature and Frequency for the Dipole Trap}
\textbf{Frequency Fixed}
...
\begin{equation}
    \frac{1}{T} \le \bigg( \frac{2 z_{max}}{g} + \frac{1}{\omega^{2}} \bigg) \frac{k}{M} \bigg( \frac{\mu_{B} g_{F_{eff}} m_{F_{eff}} \eta}{\gamma  \bra{e} \mu_{z} \ket{g}} \bigg)^{2}  \frac{c}{6 \mu_{0} I}
\end{equation}

Let's suppose that the maximum free-fall distance is $z_{max}=30cm$, and the trap frequency is $\omega = 4\pi*10kHz$. Besides, we choose $I=\frac{1W}{100 cm^{2}}$, and $\gamma=0.3$.  For the rest of the parameters, let's use the values chosen in Fig. \ref{precision_vs_power_figure}. Therefore, the condition in the temperature becomes
...
Therefore, the solution in Eq. \ref{gravimetry_signal_irradiance_dipole_trap} is valid when the temperature lies in the range

\begin{equation}
    0.15 pK \le T < 0.5 \mu K.
\end{equation}
...
The result in Eq. \ref{gravimetry_signal_irradiance_dipole_trap_general} is valid for any frequency and temperature values. Nonetheless, we still have to make sure that the condition in Eq. \ref{dipole_trap_frequency_length_condition} is satisfied when choosing the final width $\gamma \Delta Z'$ in Eq. \ref{expansion_time_dipole_trap_general}. We already found in Eq. \ref{optimal_expansion_time_temperature_fixed} the optimal expansion time when choosing this final width. Thus, the condition becomes

\begin{equation}
    0.9ms \leq \sqrt{\frac{2 z_{max}}{g}} = t_{max}.
\end{equation}

If we choose a maximum free-fall distance of $z_{max}=30cm$, then, we have

\begin{equation}
    t_{max} = 78ms.
\end{equation}

Thus, the condition is satisfied without any problem.
\end{comment}
...
we have to notice that since there exists a linear relation between $g$ and $\Delta \phi$ (Eq. \ref{gravimetry_signal_irradiance}), there also exists a linear relationship between the uncertainty $\sigma_{g}$ and the uncertainty $\sigma_{_{\Delta \Phi}}$, i.e.,
...
If we use the expansion of the atomic wave function from the dipole trap, the precision can be computed using Eq. \ref{gravimetry_signal_irradiance_dipole_trap} or Eq. \ref{gravimetry_signal_irradiance_dipole_trap_general}. Thus, we can compute the sensitivity in the measurement of $g$ as we did before. The result has the exact same form, i.e.,

\begin{equation}\label{sensitivity_eq_dipole}
    \frac{\sigma_{g}}{g} = \frac{1}{b_{d_{\omega, T}} g SNR}.
\end{equation}

where the parameter $b_{d_{\omega, T}}$ will be $b_{d_{\omega}}$ when the frequency is kept fixed (Eq. \ref{gravimetry_signal_irradiance_dipole_trap}) or $b_{d_{T}}$ when the temperature is maintained fixed (Eq. \ref{gravimetry_signal_irradiance_dipole_trap_general}).
...
In summary, if we stay during the experiment in a narrow region within the region set by $\Delta Z'$ (see Fig. \ref{selection_range_figure}), then, our method will not depend on the initial position of the atom, so we can neglect the error introduced by the detuning at least at first order.
