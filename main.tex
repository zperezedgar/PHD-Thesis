...
The above equation contains several corrections to the phase computed by using Eq.\ref{semi_classical_phase} which was obtained by using a semi-classical approach. The semi-classical result (Eq.\ref{gravimetry_signal}) can be reproduced by ignoring all the terms in Eq.\ref{quantum_phase} except the first two\footnote{Actually, all the terms without a dependency on $q_{2}$ could also be included, since their contribution to the phase will be zero (like the last term). For simplicity, we will omit those terms for which the contribution to the phase will be trivially zero.}, i.e., by taking the phase to be

\begin{equation}\label{approx_quantum_phase}
\varphi_{\pm}(x, \tau) \approx -\tau \bigg(q_{1} + \frac{\epsilon}{3} q_{2}^{2} \tau^{2}\bigg) + \left[\frac{\sigma_{x}(0)}{\sigma_{x}(\tau)} \right]^{2} (\rho_{0} - q_{2} \tau)(x-\rho_{0} \epsilon \tau)
\end{equation}

where the term $q_{2}$, defined in Eq.\ref{q1_q2}, contains information about the difference in acceleration for each of the states in the superposition, as can be seen from Eq.\ref{q2_a}, for that reason, we have used a sub-index to indicate whether we are considering $q_{2}$ with the plus or minus sign in front of the constant $\gamma_{1}$. 
...
Therefore, the phase difference accumulated at the end of the first interval, and evaluated at the position $x_{d}$ will be given by

\begin{equation}
\Delta \Phi_{T_{1}}(x_{d}) = \varphi_{+}(x_{d}, \tau) - \varphi_{-}(x_{d}, \tau)
\end{equation}

whereas for the second interval, evaluated at $x_{d}$, we have

\begin{equation}
\Delta \Phi_{T_{2}}(x_{d}) = [\varphi_{-}(x_{d}, 3\tau) - \varphi_{-}(x_{d}, \tau)] - [\varphi_{+}(x_{d}, 3\tau) - \varphi_{+}(x_{d}, \tau)]
\end{equation}

and finally, for the last interval, evaluated at $x_{d}$, we have

\begin{equation}
\Delta \Phi_{T_{3}}(x_{d}) = [\varphi_{+}(x_{d}, 4\tau) - \varphi_{+}(x_{d}, 3\tau)] - [\varphi_{-}(x_{d}, 4\tau) - \varphi_{-}(x_{d}, 3\tau)]
\end{equation}
...
\Delta \Phi (x_{d}) = \Delta \Phi_{T_{1}}(x_{d}) + \Delta \Phi_{T_{2}}(x_{d}) + \Delta \Phi_{T_{3}}(x_{d})
\end{equation}
...
The last approximation needed to reproduce the semi-classical result is the following

\begin{equation}\label{sigma_quotient_approx}
\left[\frac{\sigma_{x}(0)}{\sigma_{x}(\tau)} \right]^{2} \approx 1
\end{equation}

Finally, by using Eqs.(\ref{approx_quantum_phase}-\ref{pre_total_quantum_phase}), the definitions Eqs.(\ref{epsilon}-\ref{gamma_1}) alongside the approximations in Eqs. \ref{kappa_approx}, \ref{gamma_1_approx}, and \ref{sigma_quotient_approx}, we obtain

\begin{equation}
\Delta \Phi_{T_{1}}(x_{d}) = -x_{d}\tau + \frac{\eta \mu \tau^{2} p_{0}}{M \hbar \Delta W^{2}} - \frac{2 \eta \mu \tau^{3} g_{0}}{3 \hbar \Delta W^{3}}
\end{equation}

\begin{equation}
\Delta \Phi_{T_{1}}(x_{d}) = 2x_{d}\tau -\frac{8\eta \mu \tau^{2} p_{0}}{M \hbar \Delta W^{2}} + \frac{52 \eta \mu \tau^{3} g_{0}}{3 \hbar \Delta W^{3}}
\end{equation}

\begin{equation}
\Delta \Phi_{T_{1}}(x_{d}) = -x_{d}\tau + \frac{7\eta \mu \tau^{2} p_{0}}{M \hbar \Delta W^{2}} - \frac{74 \eta \mu \tau^{3} g_{0}}{3 \hbar \Delta W^{3}}
\end{equation}
