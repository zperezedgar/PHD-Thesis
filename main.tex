...
\kappa \approx g_{s}\frac{\mu_{z}}{\hbar} \frac{\eta}{\Delta W}
\end{equation}

where we have used the approximation $\mu_{B} \approx \mu_{z}$, where $\mu_{z}$ is the total magnetic moment of the atom along the z-direction. Additionally, by considering the alkali atoms to be $\ce{^{87}_{}Rb}$ atoms. We have the following values for this atom \cite{KAUSHALSK1970,Bunge1993} 
...
\begin{equation}
\Delta \Phi_{\tau_{1}}(x_{d}) = -x_{d}\tau + \frac{\eta \mu_{z} \tau^{2} p_{0}}{M \hbar \Delta W^{2}} - \frac{2 \eta \mu_{z} \tau^{3} g}{3 \hbar \Delta W^{3}}
\end{equation}

\begin{equation}
\Delta \Phi_{\tau_{2}}(x_{d}) = x_{d}\tau + \frac{4 \eta \mu_{z} \tau^{3} g}{3 \hbar \Delta W^{3}}
\end{equation}

\begin{equation}
\Delta \Phi_{\tau_{3}}(x_{d}) = -\frac{\eta \mu_{z} \tau^{2} p_{0}}{M \hbar \Delta W^{2}} + \frac{10 \eta \mu_{z} \tau^{3} g}{3 \hbar \Delta W^{3}}
\end{equation}

\begin{equation}\label{quantum_gravimetry_signal}
\Delta \Phi = 4 \frac{\mu_{z} \eta }{\hbar} g \bigg(\frac{\tau}{\Delta W}\bigg)^{3} = 4 \frac{\mu_{z} \eta }{\hbar} g t^{3},
\end{equation}
...
\begin{equation}
\Delta \Phi = \frac{1}{\left(1 -10 \beta ^2 + 9 \beta ^4\right)} \frac{\mu_{z} \eta}{\hbar}
\Bigg[ 4 g t^3
...
\Delta \Phi ^{(1)} =  \frac{\mu_{z} \eta}{\hbar}
\Bigg[ 4 g t^3
...
where $\mu_{B}$ is the Bohr magneton and $g_{F}$ is the Landé g-factor. Moreover, due to the linear dependence of the magnetic field with the position, the Zeeman split will depend on the position as well, i.e.,
...
\begin{equation}
  \Delta \omega(z, m_{F}) = \frac{\mu_{B} g_{F} m_{F} \eta}{\hbar} z.
\end{equation}
...
\begin{equation}
  \omega_{21}^{\prime} = \omega_{21} + \Delta \omega(z_{0}, m_{F, e}) -  \Delta \omega(z_{0}, m_{F, g}) =  \omega_{21} + \frac{\mu_{B} g_{F} m_{F_{eff}} \eta}{\hbar} z_{0},
\end{equation}

where we have defined an effective quantum magnetic number given by

\begin{equation}
    m_{F_{eff}} \equiv m_{F, e} - m_{F, g}
\end{equation}

Notice that if we choose a level with $m_{F}=0$ as the ground state, then, this state will not have a change in energy due to the Zeeman effect, and we will simply have $m_{F_{eff}} = m_{F, e}$.
...
\begin{equation}
  \omega_{B} =  \omega_{21} + \frac{\mu_{B} g_{F} m_{F_{eff}} \eta}{\hbar} z.
\end{equation}
...
\begin{equation}\label{detunig_atom_width}
  \delta (z) = \omega_{B} - \omega_{21}^{\prime} = \frac{\mu_{B} g_{F} m_{F_{eff}} \eta}{\hbar} (z-z_{0}).
\end{equation}
...
\begin{equation}
  \delta (2Z_{HWHM}) = 2\frac{\mu_{B} g_{F} m_{F_{eff}} \eta}{\hbar} Z_{HWHM}
\end{equation}

i.e.,

\begin{equation}\label{position_at_zhwhm}
  2Z_{HWHM} = \delta (2Z_{HWHM}) \frac{\hbar}{\mu_{B} g_{F} m_{F_{eff}} \eta}.
\end{equation}
...
\begin{equation}
\Delta Z = \frac{\sqrt{4 \pi^{2} - \Omega^{2} t^{2}}}{t} \frac{\hbar}{\mu_{B} g_{F} m_{F_{eff}} \eta}.
\end{equation}
...
\Delta Z = \sqrt{3} \Omega \frac{\hbar}{\mu_{B} g_{F} m_{F_{eff}} \eta}.
\end{equation}
...
\Delta Z_{0} = \sqrt{3} \Omega_{0} \frac{\hbar}{\mu_{B} g_{F} m_{F_{eff}} \eta},
\end{equation}
...
Therefore, the expanded wave packet will have a width $4$ times larger than the initial width, i.e.,

\begin{equation}\label{ration_optimial_initial_width_vs_final_width}
    \frac{(\Delta Z_{0})^{\ast}}{(\Delta Z(t))_{min}} = \frac{1}{4}.
\end{equation}
...
\begin{tabular}{||c | c | c | c | c||} 
 ...
 \hline\hline
 $\Delta Z(t)$ & $t$ & $\Delta Z_{0}$ & $(\Delta Z_{0})^{\ast} = \sqrt{\frac{\hbar t }{2M}}$ & $(\Delta Z(t))_{min} = \kappa \sqrt{\frac{\hbar t }{M}}$ \\
 \hline
...
Once the wave packet has been expanded starting from $\Delta Z_{0}$, according to Eq. \ref{initial_width}, the final width will have a Rabi frequency $\Omega_{2}$ associated with it. This new Rabi frequency can be written in terms of the Rabi frequency corresponding to the initial width. For example, if we substitute Eq. \ref{initial_width_in_terms_of_rabi} into Eq. \ref{ration_optimial_initial_width_vs_final_width}, we get

\begin{equation}\label{width_evaluated_optimal_min_rabi}
    (\Delta Z(t))_{min} = 4 \sqrt{3} \Omega_{0}^* \frac{\hbar}{\mu_{B} g_{F} m_{F_{eff}} \eta} = \sqrt{3} \Omega_{f} \frac{\hbar}{\mu_{B} g_{F} m_{F_{eff}} \eta}
\end{equation}

where $\Omega_{0}^*$ is the Rabi frequency corresponding to the optimal initial width $(\Delta Z_{0})^{\ast}$ and we have defined the Rabi frequency corresponding to the final width as

\begin{equation}\label{rabi_freqs_relation1}
    \Omega_{f} \equiv 4 \Omega_{0},
\end{equation}

This equation is analogue to Eq. \ref{ration_optimial_initial_width_vs_final_width}.
Thereby, in the same way, if we define the Rabi frequency associated with the final width for Eq. \ref{ratio_optimial_initial_width_vs_final_width_time_max}, we get

\begin{equation}\label{rabi_freqs_relation2}
    \Omega_{f} \equiv \sqrt{2} \Omega_{0}.
\end{equation}

Thus, we can characterize the expansion of the wave packet by using solely the relation between the initial and final Rabi frequency corresponding to the initial and final width of the wave packet.
...
Therefore, when designing the experiment, we have to make sure that the above constraints hold at any time that we apply a pulse to induce Rabi transitions and let the wave packet expand. 

\subsubsection{Determination of the Precision}
...
where $\boldsymbol{\mu}$ is the total magnetic moment of the atom given by the spin, orbital, and nuclear contributions, i.e.,
...
We can compute the selection range given by this Rabi frequency using Eq. \ref{initial_width}, i.e.,
...
\Delta Z' = - \sqrt{\frac{6 \mu_{0} P}{c A}} \frac{\bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F} m_{F_{eff}} \eta}.
\end{equation}
...
\begin{equation}
    \Delta Z_{f} = \gamma \Delta Z' \mathrm{,} \quad \gamma \leq 1,
\end{equation}
...
\begin{equation}\label{total_experiment_time}
    t_{max} = \frac{6 M \mu_{0} P}{\hbar c A} \bigg(\frac{ \gamma  \bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F} m_{F_{eff}} \eta}\bigg)^{2}.
\end{equation}
...
\begin{equation}
    b \equiv \frac{1}{2} \frac{ \mu_{z}}{ \hbar^{4} \eta^{5}} \bigg(\frac{3M \mu_{0}}{c} \bigg)^{3} \bigg(\frac{\bra{e} \mu_{z} \ket{g}}{\mu_{B}g_{F} m_{F_{eff}}} \bigg)^{6} \gamma^6,
\end{equation}
...
\subsubsection{Initial Cloud Width Selection}
\begin{comment}
...
\begin{figure}
    ...
    \begin{subfigure}{1\textwidth}
        ...
        \caption{Atomic wave functions before the width selection process. En realidad no conocemos la forma y ancho inicial de la funcion de onda pero podemos intuir que tiene forma gaussiana. el ancho de los atomos atrapados puede depender de la temperatura? y ser menor que el ancho seleccionado?}
...
where the uncertainty in the phase difference can be computed using Eq. \ref{gravimetry_signal_irradiance}, i.e.,

\begin{equation}
    \sigma_{_{\Delta \Phi}} = b \sigma_{g} I^3.
\end{equation}

Thus, using Eqs. \ref{gravimetry_signal_irradiance} and \ref{SNR_mid_fringe}, we get
...
Thereby, using the last two equations we get the sensitivity of the measurement
