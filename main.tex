...
For now, we will suppose that the atom is placed in the origin of our coordinate system, i.e., $z_{0}=0$. In the next section, when analyzing the case of an atomic cloud we will see what happens if the atomic wave function has an offset from the origin. Thus, we can write
...
Finally, since the electric field is several order of magnitude larger than the magnetic field, it dominates, and the Rabi flopping frequency is usually taken to be
...
\subsection{Initial Atomic Cloud Preparation and Initial Width Selection}
...
If we choose the origin of our coordinate system to be the position where the detuning is zero, then, in the case of a cloud of Alkali atoms, we can expect that many atoms in the sample will have a slight offset from the origin.

\subsection{Initial Width Selection}
In the last sections, we showed how to estimate the initial atomic width of the alkali atoms to be used during the interferometry process. Then, we saw what happens when the atom is shifted from the origin. In this section, we will study the constraints that choosing a particular initial atomic width will impose in the time that we can let the atomic wave packet expand without affecting the Rabi transitions due to the detuning.
