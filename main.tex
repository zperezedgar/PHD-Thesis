...
Let us notice that the value of $\Delta Z_{0}$ that minimizes the expansion of the wave packet (Eq. \ref{final_width_dimensional}) is given by

\begin{equation}
    (\Delta Z_{0})_{min} = \sqrt{\frac{\hbar}{2M} t}.
\end{equation}
...
\begin{equation}\label{final_width_minimum}
    (\Delta Z(t))_{min}  = \kappa \sqrt{\frac{\hbar}{M} t}.
\end{equation}

In other words, given an initial width $\Delta Z_{0}$, the expansion of the wave packet will be minimum if we let it expand for a maximum time of
...
\begin{equation}\label{width_evaluated_t_min_rabi}
    \Delta Z(t_{min}) = \kappa \sqrt{6} \Omega_{0} \frac{\hbar}{\mu_{B} g_{F} m_{F} \eta} \equiv \sqrt{3} \Omega_{2} \frac{\hbar}{\mu_{B} g_{F} m_{F} \eta} \approx \frac{\sqrt{6} \Omega_{0}}{m_{F} \Delta W},
\end{equation}

where we have used defined a new Rabi frequency $\Omega_{2}$ as

\begin{equation}
    \Omega_{2} = \sqrt{2} \kappa \Omega_{0},
\end{equation}

and we have used the approximation for $\kappa$ in Eq. \ref{kappa_approx}. The Rabi frequency $\Omega_{2}$ can be imagined, using Eq. \ref{initial_width}, as the frequency associated to the final wave packet, after it has been expanded. 
...
Therefore, when designing the experiment, we have to be careful that the above constraints hold any time we apply a pulse to induce Rabi transitions and let the wave packet expand.
