...
\begin{equation}\label{width_evaluated_optimal_min_rabi}
    (\Delta Z(t))_{min} = 4 \sqrt{3} \Omega_{0}^* \frac{\hbar}{\mu_{B} g_{F} m_{F} \eta} = \sqrt{3} \Omega_{f} \frac{\hbar}{\mu_{B} g_{F} m_{F} \eta}
\end{equation}
...
\Delta Z' = - \sqrt{\frac{6 \mu_{0} P}{c A}} \frac{\bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F} m_{F} \eta}.
\end{equation}
...
where the smaller the value of $\alpha$, the better the chances to drive the expected transitions. Now, we can use Eq. \ref{ratio_optimial_initial_width_vs_final_width_time_max} to compute the initial width that will maximize the time to reach this final width, i.e.,
...
\begin{equation}\label{total_experiment_time}
    t_{max} = \frac{6 M \mu_{0} P}{\hbar c A} \bigg(\frac{ \gamma  \bra{e} \mu_{z} \ket{g}}{\mu_{B} g_{F} m_{F} \eta}\bigg)^{2}.
\end{equation}
...
where (see Eq. \ref{pulses} and Fig. \ref{phase_graph})

\begin{equation}\label{t_max_vs_T}
    4 T = t_{max}.
\end{equation}

Thereby, using Eqs. \ref{total_experiment_time}-\ref{t_max_vs_T}, we obtain

\begin{equation}
    \Delta \Phi = b g \gamma^6 P^3, 
\end{equation}

where we have defined the parameter

\begin{equation}
    b \equiv \frac{27}{2} \frac{ \mu_{z}}{ \hbar^{4} \eta^{5}} \bigg(\frac{M \mu_{0}}{c A} \bigg)^{3} \bigg(\frac{\bra{e} \mu_{z} \ket{g}}{\mu_{B}g_{F}m_{F}} \bigg)^{6}
\end{equation}
