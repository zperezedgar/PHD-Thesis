...
\title{Gravimetry within a magnetic gradient field}
\author{Edgar Zuniga}
...
(\textcolor{blue}{Comparar este resultado con el resultado de gravimetria clasica en el que la señal va como T**2 (T al cuadrado)})
...
\textcolor{blue}{Agregar discusión de relación entre q1, q2 con a1,a2 y c+- del calculo clasico}
...
where were we have used $\tau\equiv \Delta W t$. We can notice that this result is independent of the momentum k and that it coincides with the result obtained in position space as expected.
...
where $k_{0}$ is the center of the distribution and $C$ is a scale factor that we can use to adjust the amplitude of the wave packet. In order to compute the integral in Eq. \ref{free_wave_packet_momentum_space_integral}, we need to establish the dependence between $\omega$ and $k$, i.e., the dispersion relation. We will consider the dispersion relation to be sharply peaked about $k=k_{0}$ and use a Taylor expansion about this point to second order, i.e.,
...
\begin{multline}\label{free_wave_packet_position_space_centered_0} 
    \Psi (z, t) = C \sqrt{2 \pi} (\alpha^{2} + 4\beta^{2}t^{2})^{-1/4} \exp \left[-\frac{1}{2} \frac{\alpha (z - \upsilon_{g} t)^{2}}{\alpha^{2} + 4\beta^{2}t^{2}}  \right] \\ \exp \left[i \bigg(k_{0}z - w_{0}t + \frac{\beta t}{\alpha^{2} + 4\beta^{2}t^{2}} - \frac{1}{2} \arctan\bigg[\frac{2 \beta t}{\alpha}\bigg]\bigg) \right],
\end{multline}
...
\begin{multline}\label{free_wave_packet_position_space_centered_z0} 
    \Psi (z, t) = C \sqrt{2 \pi} (\alpha^{2} + 4\beta^{2}t^{2})^{-1/4} \exp \left[-\frac{1}{2} \frac{\alpha \big(z - (z_{0} + \upsilon_{g} t)\big)^{2}}{\alpha^{2} + 4\beta^{2}t^{2}}  \right] \\ \exp \left[i \bigg(k_{0}(z-z_{0}) - w_{0}t + \frac{\beta t}{\alpha^{2} + 4\beta^{2}t^{2}} - \frac{1}{2} \arctan\bigg[\frac{2 \beta t}{\alpha}\bigg]\bigg) \right].
\end{multline}

Now, before proceeding with the comparison between this free wave packet and the wave packet of alkali-metal atoms, we will take advantage of the freedom to adjust the scale factor $C$ to modify the initial amplitude of the wave packet and get a convenient factor that will help us lately. For that reason, we will consider $C$ to be given by

\begin{equation}
C = \frac{(2 \pi)^{-3/4}}{(\alpha /2)^{-1/4}}
\end{equation}

Then, if we substitute this value and re-accommodate terms, we can write the equation describing the evolution of the free wave packet as

\begin{multline}\label{free_wave_packet_position_space_centered_z0_final_form} 
    \Psi (z, t) = \left[\frac{1}{2 \pi \big(\sigma_{z}(t)\big)^2} \right]^{1/4} \exp \left[-\frac{1}{4} \frac{ \big(z - z_{0} + \upsilon_{g} t \big)^{2}}{\big(\sigma_{z}(t)\big)^{2}} \right] \exp \left[-\frac{i}{2} \arctan\Bigg(\frac{4 \beta t}{\big(\sigma_{z}(0)\big)^{2}}\Bigg) \right] \\ \exp \left[i \bigg(k_{0}(z-z_{0}) - w_{0}t + \frac{4 \beta t}{ [\sigma_{z}(t)\sigma_{z}(0)]^{2}} \bigg) \right],
\end{multline}

where we have defined the standard deviation of the wave packet as

\begin{equation}
\sigma_{z}(t) = \sqrt{\frac{\alpha^{2} + 4\beta^{2}t^{2}}{\alpha / 2}}.
\end{equation}

Thereby the initial width of the free wave packet is given by

\begin{equation}
\sigma_{z}(0) = \sqrt{2 \alpha},
\end{equation}

Hence, we can re-write the standard deviation as

\begin{equation}\label{free_wave_packet_width} 
\sigma_{z}(t) = \sqrt{[\sigma_{z}(0)]^{2} + \left[\frac{4 \beta t}{\sigma_{z}(0)} \right]^{2}},
\end{equation}
...
\begin{tabular}{||c | c||} 
 ...
 \hline\hline
 $\sigma_{z}(t) = \sqrt{\frac{\alpha^{2} + 4\beta^{2}t^{2}}{\alpha / 2}}$ & $\sigma_{x}(\tau)$ \\ 
 \hline
 $\sigma_{z}(0) = \sqrt{2 \alpha}$ & $\sigma_{x}(0) = \kappa \Delta Z(0)$ \\
 \hline
 $4 \beta = 4 \Big(\frac{\partial^2 \omega(k)}{\partial k^2}\Big)_{k=k_{0}} = 4 \frac{\hbar}{M}$ & $\epsilon \Delta W = \frac{\hbar}{2 M} \kappa^{2}$ \\
 \hline
...
We will consider transitions between two hyperfine states levels of the atom. We choose the ground state to be the state with a total magnetic number $m_{j}=0$ while the excited state will be that with a value $m_{j}=1$. The transitions between this two-level system is characterized by a frequency  $\omega_{21}$, i.e.,

\begin{equation}
  \omega_{21} = \frac{E_{e}-E_{g}}{\hbar}.
\end{equation}

Since the atom is placed inside this external magnetic field, its energy levels will be split according to the Zeeman effect, i.e.,

\begin{equation}
  \Delta E = \mu_{B} g_{j} m_{j} B,
\end{equation}

where $\mu_{B}$ is the Bohr magneton and $g_{j}$ is the Landé g-factor. Notice how the ground state will not have a change in energy due to the Zeeman effect because $m_{j}=0$ for that state. Moreover, due to the linear dependence of the magnetic field with the position, the Zeeman split will depend on the position as well, i.e.,

\begin{equation}
  \Delta E(z) = \mu_{B} g_{j} m_{j} \eta z,
\end{equation}
...
\begin{equation}
  \Delta \omega(z) = \frac{\mu_{B} g_{j} m_{j} \eta}{\hbar} z.
\end{equation}
...
where $m_{j} = 1$ because only the excited state has a change in energy due to the Zeeman effect, however, we will continue writing only $m_{j}$ for clarity.
...
\begin{equation}
  \omega_{B} =  \omega_{21} + \frac{\mu_{B} g_{j} m_{j} \eta}{\hbar} z.
\end{equation}
...
\begin{equation}\label{detunig_atom_width}
  \delta (z) = \omega_{B} - \omega_{21}^{\prime} = \frac{\mu_{B} g_{j} m_{j} \eta}{\hbar} (z-z_{0}).
\end{equation}
...
If we consider the wave function of the atom to be symmetric around $z_{0}$, for example by considering a gaussian like wave packet, we can define the half width at half maximum and name this point as $Z_{HWHM}$ (see Fig. \ref{atom_wave_func_pos_space}). Then, the width of our atom will be given by

\begin{equation}
  \Delta Z = 2|Z_{HWHM} - z_{0}|.
\end{equation}

Without loss of generality, we can choose the coordinate system such that $z_{0}=0$. Thus, we can write

\begin{equation}\label{width_zhwhm}
  \Delta Z = 2Z_{HWHM}.
\end{equation}

Now, if we evaluate the detuning at $z = Z_{HWHM}$, we get

\begin{equation}
  \delta (Z_{HWHM}) = \frac{\mu_{B} g_{j} m_{j} \eta}{\hbar} Z_{HWHM}
\end{equation}

i.e.,

\begin{equation}\label{position_at_zhwhm}
  Z_{HWHM} = \delta (Z_{HWHM}) \frac{\hbar}{\mu_{B} g_{j} m_{j} \eta}.
\end{equation}
...
Now, a physical argument comes into play. We need to evaluate the detuning at $z_{HWHM}$. In order to do so, we will suppose that the detuning evaluated at $z_{HWHM}$ is such that the probability of transition at this position  is zero, i.e.,

\begin{equation}\label{detuning_condition}
\sqrt{\big[ \delta (z_{HWHM}) \big]^{2} + \Omega^{2}} \bigg( \frac{t}{2} \bigg) = \pi.
\end{equation}

Furthermore, we will take the probability of transition for positions $|z| \ge Z_{HWHM}$ to be zero. This approximation and the exact form of the transition probability versus the detuning can be seen in the Fig. \ref{ce2_VS_detuning}.

\begin{figure}
     \centering
     \begin{subfigure}[b]{0.9\textwidth}
         \centering
         \includegraphics[width=\textwidth]{ce2_VS_detuning_general.png}
         \caption{Exact form of the probability distribution for the excited state as function of the detuning.}
         \label{fig:ce2_VS_detuning_general}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.9\textwidth}
         \centering
         \includegraphics[width=\textwidth]{ce2_VS_detuning_cut.png}
         \caption{Approximation used to compute the detuning at $z= z_{HWHM}$ (Eq. \ref{detuning_condition}) and therefore the initial width of the atomic wave function (Eqs. \ref{width_zhwhm} and  \ref{position_at_zhwhm}). In this approximation the probability of finding the atom in the excited state is zero for positions $|z| \ge Z_{HWHM}$.}
         \label{fig:ce2_VS_detuning_cut}
     \end{subfigure}
     \hfill
     \caption{Probability of transition of the atom from the ground state to the excited state versus the detuning ($\Omega t$ is maintained fixed and the distribution is centered around $z_{0}=0$).}
     \label{ce2_VS_detuning}
\end{figure}

This approximation makes sense because the detuning evaluated at positions farther away from the position that defines the width of the atom ($z=Z_{HWHM}$) should be such that the probability of transition is always zero. In other words, we cannot induce transitions in the atom if the perturbation is applied in a position where the probability of finding the atom is zero.

We can use the Eq. \ref{detuning_condition} to determine the detuning at $z_{HWHM}$, i.e.,

\begin{equation}\label{detuning_at_zhwm}
\delta (z_{HWHM}) = \frac{\sqrt{4 \pi^{2} - \Omega^{2} t^{2}}}{t}.
\end{equation}
...
\begin{equation}
\Delta Z = 2 \frac{\sqrt{4 \pi^{2} - \Omega^{2} t^{2}}}{t} \frac{\hbar}{\mu_{B} g_{j} m_{j} \eta}.
\end{equation}
...
\begin{equation}
\Delta Z = 2 \sqrt{3} \Omega \frac{\hbar}{\mu_{B} g_{j} m_{j} \eta}.
\end{equation}

Finally, we remember that in the case of light pulses, the Rabi flopping frequency is given by

\begin{equation}
\Omega = \frac{- e E_{0} \bra{e} r \ket{g}}{\hbar},
\end{equation}

where $r$ is the position operator and $E_{0}$ is the electric field.
