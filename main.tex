...
The above equation contains several corrections to the phase computed by using Eq.\ref{semi_classical_phase} which was obtained by using a semi-classical approach. The semi-classical result (Eq.\ref{gravimetry_signal}) can be reproduced by ignoring all the terms in Eq.\ref{quantum_phase} except the first two\footnote{Actually, we can omit the last term because it does not have any dependence on the initial velocity, initial position, and/or $q_{2}$, therefore, its contribution to the phase difference will be zero. We will analyze the rest of the terms and their contribution to the phase in the next section.}, i.e., by taking the phase to be
...
\paragraph{Analysis of the Phase Terms in Position Space}
In this section, we will analyze the rest the terms in Eq.\ref{quantum_phase}, which we re-write here
...
We want to prove that it was legitimate to ignore these terms. Let us begin using Eq.\ref{xi_defs_eq_motion} to re-write the above terms as 
...
We can notice in the above equation that the last term does not have any dependence on the initial conditions (position and/or velocity) nor the acceleration. Therefore, since the initial conditions will have a dependence on $a$ (and consequently a dependence on $q_{2}$) for the second and third intervals, the contribution of the last term to the final phase difference will be trivially zero. Then, we can safely ignore the last term since its contribution will be zero
...
The contribution of these terms to the final signal is not trivial. We will study its contribution using Eq.\ref{quantum_phase_ignored_terms_simplified}  because it is easier to manipulate. At the end of the first interval, the phase due to the terms that we ignored in the last section will be given by

\begin{equation}\label{quantum_phase_ignored_terms_simplified_t1}
\varphi_{\pm, \tau_{1}}(x) = \frac{\epsilon \tau}{4 [\sigma_{x, \tau_{1}}\sigma_{x, \tau_{0}}]^{2}} \left[(x-x_{0})^{2} + 2v_{0} x_{0} \tau +a_{\pm} \tau^{2} \bigg(x+x_{0}+ \frac{a_{\pm}}{4} \tau^{2} \bigg)\right].
\end{equation}
...
\varphi_{\pm, \tau_{2}}(x) = \frac{\epsilon (2\tau)}{4 [\sigma_{x, \tau_{2}}\sigma_{x, \tau_{1}}]^{2}} \bigg[\bigg(x-(x_{0}+v_{0} \tau + \frac{a_{\pm}\tau^{2}}{2})\bigg)^{2}\\ 
...
\varphi_{\pm, \tau_{3}}(x) = \frac{\epsilon \tau}{4 [\sigma_{x, \tau_{3}}\sigma_{x, \tau_{3}}]^{2}} \bigg[\bigg(x-\Big((x_{0}+v_{0} \tau + \frac{a_{\pm}\tau^{2}}{2}) + (v_{0}+a_{\pm}\tau)(2\tau) + \frac{a_{\mp}(2\tau)^{2}}{2}\Big)\bigg)^{2}\\
...
The result obtained by considering the terms that include the standard deviation is

\begin{equation}
    \Delta \Phi \neq 0
\end{equation}

(\textcolor{red}{pareciera que los terminos que incluyen la desviación standard si deben ser ignorados?})
(\textcolor{red}{existira algun error en la ec. que describe la evolucion del paquete de onda?})
